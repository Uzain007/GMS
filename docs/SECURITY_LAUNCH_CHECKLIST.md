...
Milestone 11's web and Laravel/PostgreSQL/Redis jobs passed on commit `79ed6ae`. Both Milestone 14 CodeQL analyses passed on commit `2ddc641`. Milestone 15's quality, CodeQL and deployed-web checks passed on `45cf343`, and Vercel served that exact release. Keep the CI item unchecked until the checks are required by `main` branch protection. Keep the object-storage item unchecked after Milestone 16 passes CI until the selected production provider's encryption, IAM and lifecycle settings have their own evidence.
