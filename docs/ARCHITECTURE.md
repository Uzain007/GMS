...
Laravel exposes process liveness at `/up` and a generic PostgreSQL/Redis readiness check at `/api/v1/health/readiness`. Hosted CI also runs password recovery and tenant notifications through Redis against disposable authenticated SMTP/HTTPS boundaries, while selected-provider sandboxes remain a launch gate. The deployment runbook requires immutable releases, separate web/worker/scheduler processes, backup and restore evidence, signed-provider monitoring, centralised diagnostics and synthetic load tests that never use real member data.
