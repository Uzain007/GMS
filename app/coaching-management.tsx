...
export function CoachingManagement({ data }: { data: CoachingData }) {
  ...
  const canAssign = ["super_admin", "gym_owner", "gym_manager"].includes(data.actorRole);
  const canCreatePlan = ["super_admin", "gym_owner", "gym_manager", "trainer"].includes(data.actorRole);
  const canRecordOthers = ["super_admin", "gym_owner", "gym_manager", "trainer"].includes(data.actorRole);
  const canEditPreferences = data.actorRole === "member";
  ...
  return <section className="coaching-workspace">
    <div className="live-scope-banner"><ShieldCheck size={18} /><div><strong>Assignment-bound coaching workspace</strong><small>Plans, performance and progress stay inside the selected gym. Trainer access is verified from an active assignment by the API.</small></div><button className="icon-button" onClick={data.onReload} aria-label="Refresh coaching data"><RefreshCw size={16} /></button></div>
    ...
    {tab === "plans" && <>
      ...
      <div className="plan-grid">{data.plans.map((plan) => <article className="panel workout-plan-card" key={plan.id}><div className="plan-card-top"><span className="coaching-icon violet"><Target size={19} /></span><span className={`status ${plan.status}`}>{plan.status}</span></div><p className="eyebrow">{plan.member?.name ?? "Member plan"}</p><h3>{plan.title}</h3><p>{plan.goal ?? "Structured trainer-led progression."}</p><div className="plan-meta"><span><UserRoundCheck size={14} />{plan.trainer?.name ?? "Assigned trainer"}</span><span><CalendarCheck2 size={14} />From {plan.starts_on}</span></div><div className="exercise-list">{plan.exercises.map((exercise) => <div key={exercise.id}><span>{exercise.sort_order}</span><div><strong>{exercise.name}</strong><small>{displayTarget(exercise)}</small></div><ChevronRight size={15} /></div>)}</div>{plan.status === "active" && <button className="primary-button plan-log-button" onClick={() => setLogPlan(plan)}><Activity size={16} /> Log workout</button>}</article>)}</div>
