...
export function CoachingManagement({ data }: { data: CoachingData }) {
  ...
  return <section className="coaching-workspace">
    {!['receptionist', 'trainer', 'member'].includes(data.actorRole) && <div className="live-scope-banner"><ShieldCheck size={18} /><div><strong>{data.readOnly ? "Coaching preview" : data.actorRole === "super_admin" ? "Protected coaching workspace" : "Coaching workspace"}</strong><small>{data.readOnly ? "This sample is read-only." : data.actorRole === "super_admin" ? "Tenant and trainer-assignment authorization protect plans and progress records." : "Manage your gym’s assigned trainers, workout plans and member progress."}</small></div>{!data.readOnly && <button className="icon-button" onClick={data.onReload} aria-label="Refresh coaching data"><RefreshCw size={16} /></button>}</div>}
