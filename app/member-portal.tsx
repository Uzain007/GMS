...
export function MemberPortal({ data, actions }: { data: MemberPortalData; actions: MemberPortalActions }) {
  ...
  const paymentRestricted = Boolean(data.membership?.billing_restricted_at);
  const overdueMessage = "Your membership payment is overdue. Please contact your gym.";
  ...
  return <main className="member-shell">
    <aside className="member-sidebar"><Brand />{nav}<div className="member-sidebar-foot"><button onClick={actions.onLogout}><LogOut size={18} /> Sign out</button><small>Secure member access</small></div></aside>
    ...
    <section className="member-main">
      ...
      {data.error && data.error !== overdueMessage && <div className="member-alert error" role="alert">{data.error}<button onClick={actions.onReload}>Try again</button></div>}
      {paymentRestricted && <div className="member-alert error member-overdue-alert" role="alert">{overdueMessage}</div>}
      ...
      {data.loading && !data.profile ? <div className="member-loading"><RefreshCw className="spin" size={24} /> Preparing your member space…</div> : <div className="member-view">
        {view === "home" && <>
          <section className="member-hero"><div><span className="member-kicker">Welcome back</span><h2>{data.profile?.first_name ?? "Member"}, {paymentRestricted ? "your payment is overdue." : "you’re all set."}</h2><p>Your membership, classes and training plan are together in one private space.</p><div><button className="member-primary" disabled={!membershipAccessReady} onClick={() => navigate("pass")}><QrCode size={18} /> {membershipAccessReady ? "Open gym pass" : "Membership required"}</button><button className="member-secondary" disabled={paymentRestricted} onClick={() => navigate("classes")}><CalendarDays size={18} /> Find a class</button></div></div><Dumbbell size={94} /></section>
