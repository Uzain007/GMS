...
export function IronCoreApp() {
  ...
  if (user.platform_role === "super_admin" && !selectedGym) return <PlatformPortal data={{
    ...
    onDeleteGym: async (gymId, confirmation, reason) => { if (!api) throw new Error("The IronCore API is unavailable."); await api.deleteGym(gymId, confirmation, reason); await reloadPlatform(); },
