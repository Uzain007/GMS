...
import {
  ...
  CalendarDays, Check, ChevronDown, CircleDollarSign, Clock3, Copy, CreditCard,
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "open" : ""}`}>
      ...
      <div className="sidebar-bottom">
        {tenantMode ? <div className="tenant-security-card"><ShieldCheck size={18} /><span><strong>Gym workspace protected</strong><small>Access follows each person&apos;s assigned role</small></span></div> : <div className="capacity-card"><div className="capacity-icon"><Sparkles size={18} /></div><div><strong>96 of 100 gyms</strong><span>Platform capacity</span></div><div className="capacity-track"><span /></div><button onClick={() => onSelect("billing")}>Review plans <ArrowUpRight size={14} /></button></div>}
...
function MembersView({ query, live, onAdd }: { query: string; live?: LiveMembers; onAdd?: () => void }) {
  ...
  return <ModuleShell eyebrow="Member operations" title="Members" description={live ? "Manage member profiles, plans and access for this gym." : "A network-wide view of membership status, plans and recent joins."} action={actions}>
    {live && <div className="live-scope-banner"><ShieldCheck size={17} /><span><strong>Secure member records</strong><small>Only authorised people can view or update members from this gym.</small></span></div>}
    ...
    {activationLink && <div className="member-activation-link" role="status"><span className="activation-link-icon"><ShieldCheck size={19} /></span><div className="activation-link-content"><strong>One-time activation link ready</strong><small>Send this link to the member now. Creating another invitation revokes it.</small><div className="activation-link-controls"><input aria-label="Member activation link" readOnly value={activationLink} onFocus={(event) => event.currentTarget.select()} /><button className="secondary-button" type="button" onClick={() => void navigator.clipboard.writeText(activationLink)}><Copy size={15} /> Copy link</button></div></div><button className="icon-button activation-link-dismiss" type="button" aria-label="Dismiss activation link" onClick={() => setActivationLink(null)}><X size={17} /></button></div>}
    <section className="mini-metrics"><MiniMetric label="Total members" value={total.toLocaleString()} detail={live ? "In this gym" : "Across gym network"} /><MiniMetric label={live ? "Visible records" : "New this month"} value={live ? String(filtered.length) : "2,438"} detail={live ? "Up to 25 per page" : "12.4% increase"} /><MiniMetric label={live ? "Member access" : "At risk"} value={live ? "Protected" : "1,284"} detail={live ? "Based on assigned roles" : "Action recommended"} /></section>
...
function StandardModule({ view }: { view: View }) {
  const map: Record<string, { eyebrow: string; title: string; description: string; cards: [LucideIcon, string, string][] }> = {
    ...
    staff: { eyebrow: "Gym team", title: "Staff / Trainers", description: "Create trainers, assign their branch and manage team access.", cards: [[ShieldCheck, "Role-based access", "Give each person only the access needed for their work."], [UsersRound, "Your gym team", "Manage members, staff and trainers in one place."], [Clock3, "Activity history", "Review important account and payment changes."]] },
...
export function IronCoreDashboard({ portalMode, operator = { name: "Servion Soft", role: "Super Admin" }, activeGym, gymOptions, onGymChange, onLogout, onChangePassword, mfa, liveMembers, liveOperations, liveStaff, liveFinance, liveSaasBilling, liveEngagement, liveCoaching, liveReports, gymSettings, trainerProfile, tenantViews, onPortalSwitch, portalSwitchLabel, onCreateMember }: DashboardProps = {}) {
  ...
  const gymOverview = useMemo<GymOverviewData | null>(() => {
    ...
    return {
      ...
      actorRole: liveOperations?.actorRole ?? operator.role,
