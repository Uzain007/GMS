...
function Sidebar({ active, onSelect, open, onClose, operator, tenantViews, portalMode, activeGym }: { active: View; onSelect: (view: View) => void; open: boolean; onClose: () => void; operator: { name: string; role: string }; tenantViews?: View[]; portalMode: "platform" | "gym"; activeGym?: { id: string; name: string } }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "open" : ""}`}>
      ...
      <nav aria-label="Primary navigation">{items.map((item) => { const Icon = item.icon; return <button className={`nav-item ${active === item.id ? "active" : ""}`} key={item.id} onClick={() => { onSelect(item.id); onClose(); }}><Icon size={19} /><span>{item.label}</span>{item.id === "payments" && <span className="nav-count">12</span>}</button>; })}</nav>
...
function Header({ title, currency, setCurrency, onMenu, query, setQuery, operator, activeGym, gymOptions, onGymChange, onLogout, portalMode, onPortalSwitch, portalSwitchLabel }: { title: string; currency: Currency; setCurrency: (value: Currency) => void; onMenu: () => void; query: string; setQuery: (value: string) => void; operator: { name: string; role: string }; activeGym?: { id: string; name: string }; gymOptions?: Array<{ id: string; name: string }>; onGymChange?: (gymId: string) => void; onLogout?: () => void; portalMode: "platform" | "gym"; onPortalSwitch?: () => void; portalSwitchLabel?: string }) {
  ...
  return <header className="topbar">
    ...
    <div className="topbar-actions">
      <label className="search-box"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={portalMode === "gym" ? "Search this gym" : "Search gyms or members"} aria-label={portalMode === "gym" ? "Search this gym" : "Search gyms or members"} /><kbd>⌘ K</kbd></label>
      ...
      <label className="currency-select"><CircleDollarSign size={17} /><select value={currency} onChange={(e) => setCurrency(e.target.value as Currency)} aria-label="Display currency">{(Object.keys(currencyMeta) as Currency[]).map((code) => <option key={code}>{code}</option>)}</select><ChevronDown size={14} /></label>
      <div className="notification-wrap"><button className="icon-button notification-button" onClick={() => setShowNotes(!showNotes)} aria-label="Notifications"><Bell size={19} /><span /></button>{showNotes && <div className="notification-popover"><div><strong>Notifications</strong><span>3 new</span></div><p><b>Payment retry needed</b><small>6 memberships at Forge Fitness</small></p><p><b>Trial ending soon</b><small>Northline Gym · 4 days left</small></p><p><b>Payout completed</b><small>Apex Athletics · AED 12,480</small></p></div>}</div>
      <button className="profile-button" onClick={onLogout} aria-label={onLogout ? "Sign out" : "Open profile"}><span>{initials}</span><span className="profile-copy"><strong>{operator.name}</strong><small>{operator.role}</small></span><ChevronDown size={14} /></button>
    </div>
  </header>;
}
...
function GymTable({ gyms, currency, query, expanded = false }: { gyms: Gym[]; currency: Currency; query: string; expanded?: boolean }) {
  ...
  return <div className="table-scroll"><table className="data-table"><thead><tr><th>Gym</th><th>Members</th><th>Plan</th><th>Revenue</th><th>Health</th><th /></tr></thead><tbody>{filtered.slice(0, expanded ? 20 : 4).map((gym) => <tr key={gym.name}><td><div className="gym-cell"><span className={`gym-logo ${gym.accent}`}>{gym.initials}</span><span><strong>{gym.name}</strong><small>{gym.location}</small></span></div></td><td><strong>{gym.members.toLocaleString()}</strong></td><td><span className="plan-pill">{gym.plan}</span></td><td><strong>{money(gym.revenueGbp, currency)}</strong><small className="table-sub">this month</small></td><td><span className={`status ${gym.status.toLowerCase()}`}><i />{gym.status}</span></td><td><button className="icon-button" aria-label={`Actions for ${gym.name}`}><MoreHorizontal size={18} /></button></td></tr>)}</tbody></table>{filtered.length === 0 && <div className="empty-state"><Search size={24} /><strong>No gyms found</strong><span>Try another search term.</span></div>}</div>;
}
...
function StandardModule({ view }: { view: View }) {
  ...
  return <ModuleShell eyebrow={item.eyebrow} title={item.title} description={item.description}><section className="feature-grid">{item.cards.map(([Icon, title, text]) => <article className="panel feature-card" key={title}><span><Icon size={21} /></span><h3>{title}</h3><p>{text}</p><button>Configure <ArrowUpRight size={15} /></button></article>)}</section></ModuleShell>;
}
