...
export type SaasBillingData = {
  ...
  onPrepareManualInvoice: (priceId: string, method: "cash" | "bank_transfer", idempotencyKey: string) => Promise<SaasBillingInvoiceRecord>;
  onManualPayment: (input: { priceId?: string; invoiceId?: string; method: "cash" | "bank_transfer"; reference?: string; paymentDate?: string; notes?: string; idempotencyKey: string; receipt?: File }) => Promise<void>;
  ...
  onReverseManualPaymentApproval?: (paymentId: string, reason: string) => Promise<void>;
  onVoidInvoice?: (invoiceId: string, reason: string) => Promise<void>;
  onReplaceInvoice?: (invoiceId: string, input: ReplaceSaasBillingInvoice) => Promise<void>;
...
function featureList(plan: SaasPlanRecord): string[] {
  ...
  return [
    ...
    Number(limits.branches ?? 0) === 1 ? "1 location · Primary location only" : `${Number(limits.branches ?? 0).toLocaleString()} locations`,
...
function PlanModal({ currency, onClose, onCreate }: { currency: Currency; onClose: () => void; onCreate: (input: NewSaasPlan) => Promise<void> }) {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-labelledby="saas-plan-title">
    ...
    <form className="modal-card saas-plan-modal" onSubmit={submit}>
      ...
      <div className="field-triple"><label>Members<input name="members" required type="number" min="1" defaultValue="2500" /></label><label>Locations<input name="branches" required type="number" min="1" defaultValue="3" /></label><label>Staff<input name="staff" required type="number" min="1" defaultValue="25" /></label></div>
...
function ManualPaymentModal({ choice, paymentOptions, onClose, onSubmit }: {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-labelledby="saas-manual-payment-title">
    ...
    <form className="modal-card" onSubmit={submit}>
      <div className="modal-heading"><span>{choice.method === "cash" ? <Banknote size={21} /> : <FileUp size={21} />}</span><div><p className="eyebrow">Platform subscription</p><h2 id="saas-manual-payment-title">{choice.method === "cash" ? "Pay by cash" : "Pay by bank transfer"} · {choice.planName}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={19} /></button></div>
      {error && <div className="form-error" role="alert">{error}</div>}
      ...
      <div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? "Submitting…" : choice.method === "cash" ? "Submit cash payment" : "Submit for review"}</button></div>
    </form>
  </div>;
}
...
function InvoiceReplacementModal({ invoice, plans, onClose, onSubmit }: {
  invoice: SaasBillingInvoiceRecord;
  plans: SaasPlanRecord[];
  onClose: () => void;
  onSubmit: NonNullable<SaasBillingData["onReplaceInvoice"]>;
}) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const choices = plans.flatMap((plan) => plan.prices
    .filter((price) => price.currency === invoice.currency && (price.active || price.id === invoice.saas_plan_price_id))
    .map((price) => ({ ...price, planName: plan.name })));
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      onClose();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The corrected invoice could not be created."); }
    finally { setBusy(false); }
  }

  return <div className="modal-layer" role="dialog" aria-modal="true" aria-labelledby="saas-replace-invoice-title"><button className="modal-scrim" onClick={onClose} aria-label="Close" /><form className="modal-card" onSubmit={submit}><div className="modal-heading"><span><FileText size={20} /></span><div><p className="eyebrow">Audited financial correction</p><h2 id="saas-replace-invoice-title">Correct and replace unpaid invoice</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={18} /></button></div>{error && <div className="form-error" role="alert">{error}</div>}<p><strong className="saas-reference">{invoice.number}</strong></p><label>Correct plan price<select name="saas_plan_price_id" required defaultValue={invoice.saas_plan_price_id ?? ""}><option value="" disabled>Select a price</option>{choices.map((price) => <option key={price.id} value={price.id}>{price.planName} · {readable(price.billing_interval)} · {money(price.amount_minor, price.currency)}</option>)}</select></label><div className="field-pair"><label>Billing period starts<input name="period_start" type="date" defaultValue={invoice.period_start?.slice(0, 10) ?? ""} required /></label><label>Due date<input name="due_date" type="date" defaultValue={invoice.due_at?.slice(0, 10) ?? ""} required /></label></div><label>Audit reason<textarea name="reason" minLength={5} maxLength={1000} required /></label><div className="modal-safety"><ShieldCheck size={17} /><span><strong>Original invoice retained</strong><small>The original is voided with its history intact, then a new numbered invoice is created. Paid invoices cannot be replaced.</small></span></div><div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy || choices.length === 0}>{busy ? "Replacing…" : "Void and create replacement"}</button></div></form></div>;
}
...
export function SaasBillingManagement({ data }: { data: SaasBillingData }) {
  ...
  const [reversing, setReversing] = useState<SaasSubscriptionPaymentRecord | null>(null);
  ...
  const nextRenewal = current?.status === "trialing"
    ...
    : current?.status === "active"
      ? current.next_billing_at ?? current.current_period_end
      : null;
  ...
  const currentInvoice = payableInvoice ?? null;
  const pendingPayment = data.payments.find((payment) => payment.status === "pending" && (!payableInvoice || payment.invoice?.id === payableInvoice.id));
  const subscriptionLabel = current?.billing_restricted_at
    ? "Restricted"
    : pendingPayment
      ? "Payment under review"
      : payableInvoice?.status === "past_due" || payableInvoice?.status === "uncollectible"
        ? "Past due"
        : payableInvoice
          ? "Payment due"
          : readable(current?.status ?? "Not subscribed");
  ...
  async function prepareManualPayment(priceId: string, planName: string, method: "cash" | "bank_transfer") {
    const busyKey = `prepare-${method}-${priceId}`;
    setBusy(busyKey); setNotice(null);
    try {
      const invoice = await data.onPrepareManualInvoice(priceId, method, requestKey());
      setManualChoice({ invoiceId: invoice.id, planName, method, amountMinor: invoice.amount_remaining_minor, currency: invoice.currency, invoiceNumber: invoice.number });
      data.onReload();
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "The subscription invoice could not be prepared."); }
    finally { setBusy(null); }
  }
  ...
  return <section className="saas-workspace">
    ...
    <section className="saas-metrics">
      <article className="panel saas-current"><span className="saas-icon violet"><Building2 size={20} /></span><div><small>Current plan</small><strong>{current?.plan_name ?? "No subscription"}</strong><p>{current ? `${money(current.amount_minor, current.currency)} / ${current.billing_interval === "monthly" ? "month" : "year"}` : "Choose a plan and an available payment method."}</p></div><span className={`status ${current?.billing_restricted_at ? "restricted" : pendingPayment ? "pending" : payableInvoice?.status ?? current?.status ?? "inactive"}`}><i />{subscriptionLabel}</span></article>
      <article className="panel saas-stat"><span className="saas-icon green"><CalendarDays size={20} /></span><small>{current?.status === "trialing" ? "Trial ends" : payableInvoice ? "Payment due" : "Next renewal"}</small><strong>{date(payableInvoice?.due_at ?? nextRenewal)}</strong><p>{current?.cancel_at_period_end ? "Cancels at period end" : payableInvoice ? `${money(payableInvoice.amount_remaining_minor, payableInvoice.currency)} outstanding` : current?.provider === "stripe" ? "Automatic collection" : current ? "Manual settlement" : "No active billing"}</p></article>
    ...
    <section className="billing-grid">{visiblePlans.map((plan, index) => {
      ...
      const monthly = plan.prices.find((item) => item.active && item.currency === data.baseCurrency && item.billing_interval === "monthly");
      const yearly = plan.prices.find((item) => item.active && item.currency === data.baseCurrency && item.billing_interval === "yearly");
      return <article className={`panel plan-card saas-plan-card ${selected || index === 1 ? "featured" : ""}`} key={plan.id}>
        ...
        <div className="saas-price-options"><span>Monthly <b>{monthly ? money(monthly.amount_minor, monthly.currency) : "Not available"}</b></span><span>Yearly <b>{yearly ? money(yearly.amount_minor, yearly.currency) : "Not available"}</b></span>{price && <span>Trial <b>{price.trial_days} days</b></span>}</div>
    ...
    {!data.loading && visiblePlans.length === 0 && <div className="empty-state panel"><Sparkles size={25} /><strong>{data.error ? "Plans temporarily unavailable" : "No active SaaS plans"}</strong><span>{data.error ? "Retry the catalogue without losing access to existing billing history." : "A super administrator can publish the first platform price."}</span><button className="secondary-button" onClick={data.onReload}>Retry</button></div>}

    <section className="panel table-scroll saas-manual-payments"><div className="panel-title"><div><p className="eyebrow">Cash and bank transfer</p><h3>{data.actorRole === "super_admin" ? "Manual payment reviews" : "Payment submissions"}</h3></div>{data.actorRole === "super_admin" && data.onExportPayments ? <div className="report-export-actions">{(["csv", "xlsx", "pdf"] as const).map((format) => <button key={format} className="secondary-button" disabled={busy !== null} onClick={() => void exportPayments(format)}><Download size={14} /> {format.toUpperCase()}</button>)}</div> : <small>IronCore subscription payments</small>}</div><table className="data-table"><thead><tr><th>Plan</th><th>Method / date</th><th>Reference</th><th>Amount</th><th>Status</th><th /></tr></thead><tbody>{data.payments.map((payment) => <tr key={payment.id}><td><strong>{payment.plan?.name ?? "SaaS plan"}</strong><small className="table-sub">{date(payment.created_at)}{payment.corrections?.length ? ` · ${payment.corrections.length} correction(s)` : ""}</small>{payment.notes && <small className="table-sub">{payment.notes}</small>}</td><td>{readable(payment.effective_method ?? payment.method)}<small className="table-sub">{payment.effective_payment_date ?? payment.payment_date ?? "No reported date"}</small></td><td><span className="saas-reference">{payment.effective_reference ?? payment.reference ?? "—"}</span></td><td><strong>{money(payment.effective_amount_minor ?? payment.amount_minor, payment.currency)}</strong>{Boolean(payment.refunded_amount_minor) && <small className="table-sub">Refunded {money(payment.refunded_amount_minor ?? 0, payment.currency)}</small>}</td><td><span className={`status ${payment.status}`}><i />{payment.status === "pending" ? "Pending review" : payment.approval_reversal ? "Approval reversed" : readable(payment.status)}</span>{payment.approval_reversal && <small className="table-sub">{payment.approval_reversal.reason}</small>}{payment.review_reason && !payment.approval_reversal && <small className="table-sub">{payment.review_reason}</small>}</td><td><div className="table-actions">{payment.has_receipt && <button className="table-action" disabled={busy === `receipt-${payment.id}`} onClick={() => void downloadReceipt(payment)}><Download size={13} /> Proof</button>}{["paid", "partially_refunded", "refunded"].includes(payment.status) && data.onLoadIronCoreReceipt && <button className="table-action" disabled={busy === `generated-${payment.id}`} onClick={() => void downloadGenerated(payment)}><Download size={13} /> IronCore receipt</button>}{data.actorRole === "super_admin" && payment.status === "pending" && <button className="table-action" onClick={() => setReviewing(payment)}>Review</button>}{data.actorRole === "super_admin" && data.onCorrectManualPayment && <button className="table-action" onClick={() => setCorrecting(payment)}>Correct</button>}{data.actorRole === "super_admin" && data.onRefundManualPayment && ["paid", "partially_refunded"].includes(payment.status) && <button className="table-action" onClick={() => setRefunding(payment)}>Refund</button>}{data.actorRole === "super_admin" && data.onReverseManualPaymentApproval && payment.status === "paid" && !payment.refunded_amount_minor && !payment.approval_reversal && <button className="table-action danger-text" onClick={() => setReversing(payment)}>Reverse approval</button>}</div></td></tr>)}</tbody></table>{data.payments.length === 0 && <div className="empty-state"><Banknote size={24} /><strong>No SaaS payment submissions</strong><span>Cash and bank-transfer submissions will appear here.</span></div>}</section>
    ...
    {reversing && data.onReverseManualPaymentApproval && <BillingActionModal title="Reverse mistaken payment approval" onClose={() => setReversing(null)} onSubmit={(reason) => data.onReverseManualPaymentApproval!(reversing.id, reason)} />}
    ...
    {replacing && data.onReplaceInvoice && <InvoiceReplacementModal invoice={replacing} plans={data.plans} onClose={() => setReplacing(null)} onSubmit={data.onReplaceInvoice} />}
