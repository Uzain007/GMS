...
export type SaasInvoiceCorrectionHistoryRecord = { id: string; action: string; old_value: Record<string, unknown> | null; new_value: Record<string, unknown> | null; reason: string | null; actor: { id: string; name: string } | null; created_at: string | null };
export type SaasBillingInvoiceRecord = { id: string; saas_plan_price_id?: string | null; number: string | null; status: "draft" | "upcoming" | "due" | "open" | "past_due" | "paid" | "void" | "cancelled" | "uncollectible"; currency: GymSummary["base_currency"]; amount_due_minor: number; amount_paid_minor: number; amount_remaining_minor: number; hosted_invoice_url: string | null; invoice_pdf_url: string | null; available_at?: string | null; period_start: string | null; period_end: string | null; due_at: string | null; grace_ends_at?: string | null; paid_at: string | null; voided_at?: string | null; void_reason?: string | null; correction_history?: SaasInvoiceCorrectionHistoryRecord[]; created_at: string | null };
export type PlatformBankTransferDetails = { account_name: string; bank_name: string; account_number_or_iban: string; routing_details: string | null; payment_instructions: string | null };
export type SaasPaymentOptions = { cash_available: boolean; bank_transfer_available: boolean; stripe_configured: boolean; platform_bank_details: PlatformBankTransferDetails | null };
...
export type SaasApprovalReversalRecord = { id: string; reason: string; reversed_by: { id: string; name: string } | null; reversed_at: string | null };
export type SaasSubscriptionPaymentRecord = { id: string; gym_id: string; saas_plan_price_id: string; gym_subscription_id: string | null; method: "cash" | "bank_transfer"; effective_method?: "cash" | "bank_transfer"; status: "pending" | "paid" | "rejected" | "partially_refunded" | "refunded" | "voided"; amount_minor: number; refunded_amount_minor?: number; effective_amount_minor?: number; currency: GymSummary["base_currency"]; reference: string | null; effective_reference?: string | null; payment_date?: string | null; effective_payment_date?: string | null; notes?: string | null; corrections?: SaasPaymentCorrectionRecord[]; refunds?: SaasPaymentRefundRecord[]; approval_reversal?: SaasApprovalReversalRecord | null; has_receipt: boolean; receipt_original_name: string | null; reviewed_at: string | null; review_reason: string | null; paid_at: string | null; submitted_by?: { id: string; name: string; email: string }; invoice?: { id: string; number: string | null; period_start: string | null; period_end: string | null; due_at: string | null } | null; plan?: { id: string; name: string; code: string; billing_interval: "monthly" | "yearly" }; created_at: string | null };
...
export type NewSaasSubscriptionPayment = { saas_plan_price_id?: string; saas_billing_invoice_id?: string; method: "cash" | "bank_transfer"; idempotency_key: string; reference?: string; payment_date?: string; notes?: string; receipt?: File };
export type NewSaasBillingInvoice = { saas_plan_price_id: string; amount_minor: number; currency: GymSummary["base_currency"]; due_date: string; idempotency_key: string; reason: string };
export type ReplaceSaasBillingInvoice = { saas_plan_price_id: string; period_start: string; due_date: string; idempotency_key: string; reason: string };
export type PrepareSaasSubscriptionInvoice = { saas_plan_price_id: string; method: "cash" | "bank_transfer"; idempotency_key: string };
...
export type PlatformSaasPaymentRecord = { id: string; gym_id: string; gym_name: string; plan_name: string | null; billing_interval: string | null; invoice_id: string | null; invoice_number: string | null; method: "cash" | "bank_transfer"; effective_method: "cash" | "bank_transfer"; status: SaasSubscriptionPaymentRecord["status"]; amount_minor: number; effective_amount_minor: number; refunded_amount_minor: number; currency: GymSummary["base_currency"]; reference: string | null; effective_reference: string | null; payment_date: string | null; effective_payment_date: string | null; notes: string | null; has_receipt: boolean; submitted_by: { id: string; name: string; email: string } | null; review_reason: string | null; correction_count: number; refund_count: number; approval_reversal: { reason: string; reversed_at: string | null } | null; created_at: string | null };
export type PlatformBillingRecord = { metrics: { total_gyms: number; active_gyms: number; trial_gyms: number; paid_gyms: number; unpaid_gyms: number; past_due_gyms: number; billing_suspended_gyms: number; cancelled_archived_gyms: number; mrr_by_currency: Record<string, number>; revenue_this_month_by_currency: Record<string, number>; outstanding_by_currency: Record<string, number>; overdue_by_currency: Record<string, number>; upcoming_renewals: number; trial_conversions: number }; subscriptions: Array<{ id: string; gym_id: string; gym_name: string; plan_id: string; plan_name: string; status: string; currency: GymSummary["base_currency"]; amount_minor: number; billing_interval: string; next_billing_at: string | null; grace_period_days: number; billing_restricted_at: string | null }>; invoices: Paginated<{ id: string; gym_id: string; gym_name: string; subscription_id: string; plan_price_id: string | null; plan_name: string | null; number: string | null; status: string; currency: GymSummary["base_currency"]; amount_due_minor: number; amount_paid_minor: number; amount_remaining_minor: number; period_start: string | null; period_end: string | null; due_at: string | null; grace_ends_at: string | null; paid_at: string | null }>; payments: Paginated<PlatformSaasPaymentRecord> };
...
export type UpdateSaasPlan = { name?: string; description?: string | null; status?: SaasPlanRecord["status"]; sort_order?: number; feature_limits?: SaasFeatureLimits; payment_methods?: SaasPaymentMethod[]; price?: Omit<NewSaasPlanPrice, "reason">; prices?: Array<Omit<NewSaasPlanPrice, "reason">>; reason: string };
...
export class IronCoreApi {
  ...
  async reverseSaasPaymentApproval(gymId: string, paymentId: string, reason: string): Promise<SaasSubscriptionPaymentRecord> {
    await this.csrf();
    return (await this.request<ApiEnvelope<SaasSubscriptionPaymentRecord>>(`/api/v1/gyms/${encodeURIComponent(gymId)}/saas-subscription/manual-payments/${encodeURIComponent(paymentId)}/approval-reversal`, { method: "POST", body: JSON.stringify({ reason }) }, gymId)).data;
  }
  ...
  async replaceSaasInvoice(gymId: string, invoiceId: string, input: ReplaceSaasBillingInvoice): Promise<SaasBillingInvoiceRecord> {
    await this.csrf();
    return (await this.request<ApiEnvelope<SaasBillingInvoiceRecord>>(`/api/v1/gyms/${encodeURIComponent(gymId)}/saas-billing-invoices/${encodeURIComponent(invoiceId)}/replace`, { method: "POST", body: JSON.stringify(input) }, gymId)).data;
  }

  async createSaasInvoice(gymId: string, input: NewSaasBillingInvoice): Promise<SaasBillingInvoiceRecord> {
    await this.csrf();
    return (await this.request<ApiEnvelope<SaasBillingInvoiceRecord>>(
      `/api/v1/gyms/${encodeURIComponent(gymId)}/saas-billing-invoices`,
      { method: "POST", body: JSON.stringify(input) },
      gymId,
    )).data;
  }

  async prepareSaasSubscriptionInvoice(gymId: string, input: PrepareSaasSubscriptionInvoice): Promise<SaasBillingInvoiceRecord> {
    await this.csrf();
    return (await this.request<ApiEnvelope<SaasBillingInvoiceRecord>>(
      `/api/v1/gyms/${encodeURIComponent(gymId)}/saas-subscription/manual-invoice`,
      { method: "POST", body: JSON.stringify(input) },
      gymId,
    )).data;
  }
  ...
  async deleteSaasPlan(planId: string, reason: string): Promise<void> {
    await this.csrf();
    await this.request<void>(`/api/v1/platform/saas-plans/${encodeURIComponent(planId)}`, { method: "DELETE", body: JSON.stringify({ reason }) });
  }
