...
type Option = { id: string; name: string; isPrimary?: boolean };
...
function branchLabel(branch: Option): string {
...
export function EngagementManagement({ data }: { data: EngagementData }) {
  ...
  // Multi-location reception must choose an explicit branch; never silently
  // admit a member at the first row returned by a tenant-scoped list.
  const defaultBranch = data.branches.length === 1 ? data.branches[0].id : "";
  ...
  async function checkIn(event: FormEvent<HTMLFormElement>) {
    ...
    const branchId = String(form.get("branch_id") ?? "").trim() || undefined;
    await act(async () => { await data.onCheckIn({ branchId, method: "member_code", accessValue }); formElement.reset(); }, "Member checked in successfully.");
  }

  async function checkInFromCamera(credentialValue: string) {
    if (!selectedCheckInBranch) {
      const message = "Select an active branch before scanning a member QR code.";
      setActionError(message);
      throw new Error(message);
    }
    setBusy(true); setActionError(null); setNotice(null);
    try {
      await data.onCheckIn({ branchId: selectedCheckInBranch || undefined, method: "qr", accessValue: credentialValue });
  ...
  return <section className="engagement-workspace">
    {!['receptionist', 'trainer', 'member'].includes(data.actorRole) && <div className="live-scope-banner"><ShieldCheck size={18} /><div><strong>{data.readOnly ? "Attendance preview" : data.actorRole === "super_admin" ? "Protected attendance workspace" : "Attendance and classes"}</strong><small>{data.readOnly ? "These sample attendance and class records are read-only." : data.actorRole === "super_admin" ? "Tenant, branch and secure QR checks protect every scan, roster and booking." : "Manage check-ins, classes and bookings for your gym."}</small></div>{!data.readOnly && <button className="icon-button" onClick={data.onReload} aria-label="Refresh engagement data"><RefreshCw size={16} /></button>}</div>}
    {data.error && <div className="form-error" role="alert">{data.error}</div>}
    {actionError && <div className="form-error" role="alert">{actionError}</div>}
    {notice && <div className="form-notice" role="status">{notice}</div>}

    <div className="engagement-metrics">
      <article className="panel engagement-stat"><span className="engagement-icon green"><DoorOpen size={19} /></span><div><small>Present now</small><strong>{present.length}</strong><p>Across this gym&apos;s branches</p></div></article>
    ...
    {tab === "attendance" && <>
      {canCheckIn && <article className="panel checkin-console">
        <div className="checkin-copy"><span className="engagement-icon violet"><QrCode size={21} /></span><div><p className="eyebrow">Front desk admission</p><h2>Scan QR or enter Member Code</h2><p>Camera scans use the secure QR pass. Member Code is the manual fallback. IronCore confirms membership and branch access before admission.</p></div></div>
        <form onSubmit={checkIn}>
          {data.branches.length === 0
            ? <label>Location<input name="branch_name" value="No active location available" readOnly /><small>Ask a Gym Admin to activate a branch before check-in.</small></label>
            : data.branches.length === 1
              ? <label>Branch<input name="branch_name" value={branchLabel(data.branches[0])} readOnly /><input name="branch_id" type="hidden" value={selectedCheckInBranch} /></label>
              : <label>Branch<select name="branch_id" value={selectedCheckInBranch} onChange={(event) => setCheckInBranch(event.target.value)} required><option value="" disabled>Select branch</option>{data.branches.map((branch) => <option key={branch.id} value={branch.id}>{branchLabel(branch)}</option>)}</select></label>}
          <label>Member Code<input ref={memberCodeInput} name="access_value" required autoComplete="off" inputMode="numeric" pattern="[0-9]{4,6}" minLength={4} maxLength={6} placeholder="e.g. 104287" /></label>
          <button className="primary-button" disabled={busy || data.loading || !selectedCheckInBranch} type="submit"><TicketCheck size={16} /> Check in with code</button>
          <button className="secondary-button" disabled={busy || data.loading || !selectedCheckInBranch} type="button" onClick={() => setScannerOpen(true)}><Camera size={16} /> Scan QR with Camera</button>
          <button className="secondary-button" type="button" onClick={() => { setCredential(null); setCredentialModal(true); }}><QrCode size={16} /> Open member QR</button>
        </form>
      </article>}
    ...
    {credentialModal && <Modal title="Member QR card" eyebrow="Secure member pass" icon={<QrCode size={20} />} onClose={closeCredentialModal}>{credential ? <div className="credential-result"><canvas ref={qrCanvas} aria-label="Persistent member QR credential" /><div className="member-code-display credential-member-code"><small>Member Code</small><strong>{issuedMember?.memberCode ?? "——"}</strong></div><strong>{issuedMember?.name ?? "Member"}</strong><p>This is the member&apos;s normal QR pass. Opening it again returns the same secure QR until an authorised user replaces it.</p><div className="credential-actions"><button className="secondary-button" type="button" onClick={() => void navigator.clipboard?.writeText(credential)}>Copy encoded credential</button><button className="secondary-button" type="button" disabled={busy} onClick={() => void rotateCredential()}><RefreshCw size={14} /> Replace QR</button></div></div> : <form onSubmit={ensureCredential}><label>Member<select name="member_id" required defaultValue=""><option value="" disabled>Select a member</option>{data.members.map((member) => <option key={member.id} value={member.id}>{member.name} · Member Code {member.memberCode}</option>)}</select></label><div className="modal-note"><ShieldCheck size={16} />This pass is protected, stays available after sign-in and can be replaced when needed.</div><div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCredentialModal}>Cancel</button><button className="primary-button" disabled={busy} type="submit">Open QR card</button></div></form>}</Modal>}
    {scannerOpen && <QrCameraScanner branchName={data.branches.find((branch) => branch.id === selectedCheckInBranch)?.name ?? "Primary Branch"} onScan={checkInFromCamera} onClose={() => setScannerOpen(false)} onManualFallback={() => { setScannerOpen(false); window.setTimeout(() => memberCodeInput.current?.focus(), 0); }} />}
  </section>;
}
