...
export function EngagementManagement({ data }: { data: EngagementData }) {
  ...
  return <section className="engagement-workspace">
    ...
    {tab === "attendance" && <>
      {canCheckIn && <article className="panel checkin-console"><div className="checkin-copy"><span className="engagement-icon violet"><QrCode size={21} /></span><div><p className="eyebrow">Front desk admission</p><h2>Scan QR or enter Member Code</h2><p>Camera scans use the secure QR pass. Member Code is the manual fallback. IronCore confirms membership and branch access before admission.</p></div></div><form onSubmit={checkIn}>{data.branches.length === 0 ? <label>Location<input name="branch_name" value="Primary location" readOnly /><input name="branch_id" type="hidden" value="" /><small>This gym&apos;s active primary location will be used. If the gym has multiple locations, select one after they finish loading.</small></label> : data.branches.length === 1 ? <label>Location<input name="branch_name" value={`${data.branches[0].name}${data.branches[0].isPrimary ? " · Primary" : ""}`} readOnly /><input name="branch_id" type="hidden" value={selectedCheckInBranch} /></label> : <label>Branch<select name="branch_id" value={selectedCheckInBranch} onChange={(event) => setCheckInBranch(event.target.value)} required><option value="" disabled>Select branch</option>{data.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}{branch.isPrimary ? " · Primary" : ""}</option>)}</select></label>}<label>Member Code<input ref={memberCodeInput} name="access_value" required autoComplete="off" inputMode="numeric" pattern="[0-9]{4,6}" minLength={4} maxLength={6} placeholder="e.g. 104287" /></label><button className="primary-button" disabled={busy || data.loading} type="submit"><TicketCheck size={16} /> Check in with code</button><button className="secondary-button" disabled={busy || data.loading} type="button" onClick={() => setScannerOpen(true)}><Camera size={16} /> Scan QR with Camera</button><button className="secondary-button" type="button" onClick={() => { setCredential(null); setCredentialModal(true); }}><QrCode size={16} /> Open member QR</button></form></article>}
    ...
    {scannerOpen && <QrCameraScanner branchName={data.branches.find((branch) => branch.id === selectedCheckInBranch)?.name ?? "Primary location resolved securely"} onScan={checkInFromCamera} onClose={() => setScannerOpen(false)} onManualFallback={() => { setScannerOpen(false); window.setTimeout(() => memberCodeInput.current?.focus(), 0); }} />}
  </section>;
}
