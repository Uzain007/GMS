...
export type NewFinancePayment = { member_id: string; membership_id?: string; invoice_id?: string; branch_id?: string; method: FinancePayment["method"]; amount_minor: number; currency: "GBP" | "USD" | "PKR" | "AED" | "SAR"; idempotency_key: string; payment_date?: string; notes?: string; bank_reference?: string; transferred_on?: string; receipt?: File };
...
export function FinancialManagement({ data, query }: { data: FinanceData; query: string }) {
  ...
  return <>
    ...
    {!['receptionist', 'trainer', 'member'].includes(data.actorRole) && <div className="live-scope-banner"><ShieldCheck size={17} /><span><strong>{data.actorRole === "super_admin" ? "Protected tenant ledger" : "Your gym finances"}</strong><small>{data.actorRole === "super_admin" ? "Tenant resolution, authorization and PostgreSQL RLS protect every financial request." : "Invoices, payments and refunds shown here belong to your gym."}</small></span></div>}
...
function Modal({ title, icon: Icon, close, submit, submitLabel, children }: { title: string; icon: typeof CreditCard; close: () => void; submit: (form: HTMLFormElement) => Promise<void>; submitLabel: string; children: React.ReactNode }) {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true"><button className="modal-scrim" onClick={close} aria-label="Close dialog" /><form className="modal-card finance-modal" onSubmit={handle}><div className="modal-heading"><span><Icon size={21} /></span><div><p className="eyebrow">Payment record</p><h2>{title}</h2></div><button type="button" className="icon-button" onClick={close}><X size={19} /></button></div>{error && <div className="form-error" role="alert">{error}</div>}{children}<div className="modal-note"><ShieldCheck size={17} />Linked member, invoice and amount details are checked before this record is saved.</div><div className="modal-actions"><button type="button" className="secondary-button" onClick={close}>Cancel</button><button disabled={busy} className="primary-button">{busy ? "Saving…" : submitLabel}</button></div></form></div>;
}
