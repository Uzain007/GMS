...
export type NewFinancePayment = { member_id: string; membership_id?: string; invoice_id?: string; branch_id?: string; method: FinancePayment["method"]; amount_minor: number; currency: "GBP" | "USD" | "PKR" | "AED" | "SAR"; idempotency_key: string; notes?: string; bank_reference?: string; receipt?: File };
...
function BankReviewModal({ payment, data, close }: { payment: FinancePayment; data: FinanceData; close: () => void }) {
  return <Modal title={`Review ${payment.receipt}`} icon={FileCheck2} close={close} submitLabel="Save decision" submit={async (form) => { const value = new FormData(form); await data.onReviewBankTransfer(payment.id, String(value.get("decision")) as "approve" | "reject", String(value.get("reason"))); }}><div className="refund-summary"><span>Transfer amount</span><strong>{money(payment.amountMinor, payment.currency)}</strong></div>{payment.bankReceipt && <button type="button" className="secondary-button receipt-download" onClick={() => void openReceipt(data.onLoadReceipt, payment.id)}><Download size={15} /> Download {payment.bankReceipt.originalName}</button>}<label>Decision<select name="decision" defaultValue="approve"><option value="approve">Approve and mark Paid</option><option value="reject">Reject payment</option></select></label><label>Audit reason<textarea name="reason" rows={4} required minLength={5} maxLength={1000} placeholder="What did you verify?" /></label><div className="payment-safety warning"><ShieldCheck size={16} /><span><strong>Protected review</strong><small>Approval settles the linked invoice. Rejection leaves its balance outstanding.</small></span></div></Modal>;
}
